...
This app now uses `jsbundling-rails` + `cssbundling-rails` (`esbuild` + `sass/postcss`), which is more compatible with modern Node versions.
Build assets, then start Rails:
...
## sec_13f_record Tasks

- Initialize two quarters:

```bash
bundle exec rake "sec_13f_record:init_two_quarters[2025,4,2026,1]"
```

- Export keyword comparison CSV (natural previous quarter rule):

```bash
bundle exec rake "sec_13f_record:export_keyword[intc]"
```

Output directory: `exports/keyword_comparisons`

Rules:
- Match keyword against `Sym / Issuer Name / Cl / CUSIP / Option Type`.
- Compare current latest quarter vs natural previous quarter only.
- If both quarters have filings but a holding is missing in one quarter, output `0` for that quarter.
- If previous natural quarter has no filing at all, output `not have data` for previous-quarter cells.
